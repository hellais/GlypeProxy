<?php
/*******************************************************************
* Glype Proxy Script: Admin Control Panel
*
...
*******************************************************************
* This is a stand-alone admin control panel for glype proxy script.
* Upload to your root proxy directory and run in your browser.
******************************************************************/
...
// And backwards compatability (will be removed at some point)
...
OUT;
      ...
      echo <<<OUT
            ...
            <div id="topheader">
               <h1 id="title">
                  <a href="{$self}">admin control panel</a>
                  <span>for glype proxy</span>
               </h1>
            </div>
...
OUT;
      ...
      <div id="footer">
         ...
         <div id="footer_bg">
            ...
            <p>Glype Control Panel &copy; 2008 by <a href="http://www.glype.com/"><b>glype</b></a></p>
         </div>
...
if ( $input->gFetch && $user->isAdmin() ) {
   ...
   switch ( $input->gFetch ) {
      ...
      // Get the latest version from glype.com
      case 'latest-version':
         echo @file_get_contents('http://www.glype.com/feeds/proxy-version.php');
         break;
      ...
      case 'news':
         ...
         // Connect to glype
         if ( $ch = curl_init('http://www.glype.com/feeds/news.php') ) {
...
if ( ! $settingsLoaded ) {
   ...
   // Show error and exit
   $error->add('The settings file for your glype proxy could not be found.
   ...
   ******************************************************************/
   ...
   case '':
      ...
      if ( ! ( $ok = version_compare($phpVersion, '5', '>=') ) ) {
         $error->add('<strong>glype proxy</strong> requires at least PHP 5 or greater.');
      }
      ...
      if ( ! ( $ok = function_exists('curl_version') ) ) {
         $error->add('<strong>glype proxy</strong> requires cURL/libcurl.');
      }
      ...
      echo <<<OUT
      <p>This script provides an easy to use interface for managing your <strong>glype proxy</strong>. Use the navigation above to get started.</p>
...
OUT;
      ...
      if ( $error->hasMsg() ) {
         echo '<p><span class="bold error-color">Environment check failed</span>. You will not be able to run glype proxy until you fix the above issue(s).</p>';
      } else {
         echo '<p><span class="bold ok-color">Environment okay</span>. You can run glype proxy on this server.</p>';
      }
      ...
      function updateLatestVersion(response) {
         document.getElementById('current-version').innerHTML = response;
      }
OUT;
      $output->addJavascript($javascript);
      $output->addDomReady("runAjax('$self?fetch=latest-version', null, updateLatestVersion);");
      ...
      <ul class="green">
         <li>Admin control panel: <b>{$acpVersion}</b></li>
         <li>Glype proxy version: <b>{$proxyVersion}</b></li>
         <li>Latest stable version: <span class="bold" id="current-version">unknown</span></li>
      </ul>
OUT;
     ...
     <iframe src="{$self}?fetch=news" style="width: 100%; height:150px; border: 1px solid #ccc;"></iframe>

OUT;

      // Add footer links
      $output->addFooterLinks('glype documentation', 'http://docs.glype.com/');
      $output->addFooterLinks('glype support forums', 'http://forums.glype.com/');
...
OUT;
      ...
      <ul class="black">
         <li><b>Temporary directory:</b> many features require write access to the temporary directory. Ensure you set up the permissions accordingly if you use any of these features: caching, logging, server-side cookies, maintenance/cleanup and the server load limiter.</li>
         <li><b>Sensitive data:</b> some temporary files may contain personal data that should be kept private - that is, log files and server-side cookies. If using these features, protect against unauthorised access by choosing a suitable location above the webroot or using .htaccess files to deny access.</li>
...
OUT;
   ...
   ******************************************************************/
   ...
   case 'cache':
      ...
      // Determine status of cache, starting with enabled/disabled
      $status = $CONFIG['use_cache'] ? '<span class="ok-color">enabled</span>' : '<span class="error-color">disabled</span>';

      // Get path to folder
      $folderPath = isset($CONFIG['cache_path']) ? $CONFIG['cache_path'] : 'not set';

      // Does the folder exist and writable?
      switch ( false ) {
         case file_exists($folderPath):
            $folderError = 'does not exist';
            break;

         case is_writable($folderPath):
            $folderError = 'is not writable';
            break;
      ...
      // Are we clearing or displaying the form?
      if ( $input->pClear && ! $folderError ) {
         ...
         while ( ($file = readdir($handle)) !== false ) {
            ...
            // Ignore dot files and undeletable files
            if ( ( isset($file[0]) && $file[0] == '.' ) || is_dir($filePath) || ! is_writable($filePath) ) {
               continue;
            }

            // Add filesize to count
            $cleared += @filesize($filePath);
         ...
         $cleared = formatSize($cleared);

         // Close handle
         closedir($handle);

         // Show success
         $confirm->add('The cache has been cleared. <b>' . $cleared . '</b> of space freed.');
         $location->redirect('cache');

      }

      // Text for folder status
      $folderStatus = $folderError ? '<span class="error-color">' . $folderError . '</span>' : '<span class="ok-color">exists and is writable</span>';
      ...
      // Total space available on disk/partion
      if ( $folderError ) {
         $total = 'n/a';
      } else if ( $tmp = @disk_total_space($folderPath) ) {
         $total = formatSize($tmp);
      } else {
      ...
      // Print page
      $output->title     = 'cache';
      $output->bodyTitle = 'Cache status';

      echo <<<OUT
         <table class="form_table" border="0" cellpadding="0" cellspacing="0">
            <tr>
               <td align="right">Cache feature:</td>
               <td><b>{$status}</b></td>
            </tr>
            <tr>
               <td align="right">Cache folder:</td>
               <td><b>{$folderPath}</b> ($folderStatus)</td>
            </tr>
            <tr>
               <td align="right"><span class="tooltip" onmouseover="tooltip('This is cumulative filesize of all cached files, which will be lower than the real space used on disk.')" onmouseout="exit()">Size of cache folder</span>:</td>
               <td><b>{$used}</b> used by <b>{$fileCount}</b> files</td>
            </tr>
            <tr>
               <td align="right"><span class="tooltip" onmouseover="tooltip('This is the total size of the disk partition on which the cache folder is located. Other limits may be imposed on your disk usage.')" onmouseout="exit()">Size of disk</span>:</td>
               <td><b>{$total}</b></td>
            </tr>
            <tr>
               <td align="right">Options:</td>
               <td>
                  <form action="{$self}?cache" method="post"><input class="button" value="Empty cache folder &raquo;" onclick="if(!confirm('This will delete all files in the cache folder. Continue?')) return false;" name="clear" type="submit"></form>
               </td>
            </tr>
         </table>


         <p class="little">Note: You can also set up your proxy to automatically empty the cache periodically with the maintenance feature.</p>
OUT;
      $output->addDomReady("document.getElementById('trim').focus();");
